...
export function FrontOfficerDashboardPage({ user }) {
  const [summary, setSummary] = useState(null);
  const [helpText, setHelpText] = useState('');
  const [error, setError] = useState('');

  useEffect(() => {
    fetch(`/api/dashboard/front-officer?userId=${encodeURIComponent(user.userId)}`)
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-4">Front Officer Dashboard</h1>
